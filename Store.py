...
class Store:
    ...
    def addLabel(self, item, number):
        if self.cart.itemLimit < 24:
            ...
            Label(self.ucartFrame, text=item.name+" x"+str(number)+": "+str(item.costDict[number]), anchor=E).pack(side=TOP,
                                                                                                                  fill=X)
            ...
            numItems = self.cart.dict[item.name] + number
            self.cart.dict[item.name] = numItems
    ...
    def createImage(self, parent, WIDTH, HEIGHT):
        ...
        self.storeFrame.pack_propagate(0)
        ...
        Button(pFrame, image=self.pbone, command= lambda: self.addLabel(self.phase, 1)).pack(side=LEFT)
        Label(pFrame, text="Cost: " + str(self.phase.cost), bg="#30F630").place(relx=.33, rely=.725)
        Button(pFrame, image=self.pbthree, command= lambda: self.addLabel(self.phase, 3)).pack(side=LEFT)
        Label(pFrame, text="Cost: " + str(self.phase.three), bg="#30F630").place(relx=.445, rely=.725)
        Button(pFrame, image=self.pbfive, command= lambda: self.addLabel(self.phase, 5)).pack(side=LEFT)
        Label(pFrame, text="Cost: " + str(self.phase.five), bg="#30F630").place(relx=.56, rely=.725)
        Label(tFrame, image=self.tbi).pack(side=LEFT)
        Button(tFrame, image=self.tbone, command= lambda: self.addLabel(self.tele, 1)).pack(side=LEFT)
        Label(tFrame, text="Cost: " + str(self.tele.cost)).place(relx=.33, rely=.725)
        Button(tFrame, image=self.tbthree, command= lambda: self.addLabel(self.tele, 3)).pack(side=LEFT)
        Label(tFrame, text="Cost: " + str(self.tele.three)).place(relx=.445, rely=.725)
        Button(tFrame, image=self.tbfive, command= lambda: self.addLabel(self.tele, 5)).pack(side=LEFT)
        Label(tFrame, text="Cost: " + str(self.tele.five)).place(relx=.56, rely=.725)
        Label(rFrame, image=self.rbi).pack(side=LEFT)
        Button(rFrame, image=self.rbone, command= lambda: self.addLabel(self.reset, 1)).pack(side=LEFT)
        Label(rFrame, text="Cost: " + str(self.reset.cost), bg="black", fg="white").place(relx=.33, rely=.725)
        Button(rFrame, image=self.rbthree, command= lambda: self.addLabel(self.reset, 3)).pack(side=LEFT)
        Label(rFrame, text="Cost: " + str(self.reset.three), bg="black", fg="white").place(relx=.445, rely=.725)
        Button(rFrame, image=self.rbfive, command= lambda: self.addLabel(self.reset, 5)).pack(side=LEFT)
        ...
        self.cartFrame.pack_propagate(0)
        ...
        self.totalLabel = Label(self.ucartFrame, textvariable=self.totalVar, anchor=W).pack(side=BOTTOM,
                                                                                           fill=X)
